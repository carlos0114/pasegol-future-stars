CREATE OR REPLACE FUNCTION public.has_explorer_access(_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE p.id = _user_id
      AND p.user_type IN ('club', 'scout')
  )
  OR public.has_club_or_scout_profile(_user_id);
$$;

REVOKE ALL ON FUNCTION public.has_explorer_access(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.has_explorer_access(uuid) TO service_role;
